<div class="col-sm-3 col-md-2 sidebar">

  <ul class="nav nav-sidebar">  
    <li style="display:none"><a href="<?php echo base_url() ?>amenities/all">Amenities</a></li>  
    <hr>
    <li><a href="<?php echo base_url() ?>spaces/all">Space</a></li>  
    <hr>
    <li>
      <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne">Home </a>
      <ul id="collapseOne" class="panel-collapse collapse ">        
        <li><a href="<?php echo base_url() ?>slider/all">Image Slider</a></li>        
      </ul>
    </li>
    <hr> 

    <li>
      <a data-toggle="collapse" data-parent="#accordion" href="#collapseTwo">Page Header Content</a>
      <ul id="collapseTwo" class="panel-collapse collapse ">        
        <li><a href="<?php echo base_url() ?>page/header/gallery">Gallery</a></li>        
        <li><a href="<?php echo base_url() ?>page/header/team">Team</a></li>        
        <li><a href="<?php echo base_url() ?>page/header/profile">Profile</a></li>        
        <li style="display:none;"><a href="<?php echo base_url() ?>page/header/team">Team</a></li>        
        <li><a href="<?php echo base_url() ?>page/header/catering">Catering</a></li>        
        <li><a href="<?php echo base_url() ?>page/header/catering_partners">Catering Partners</a></li>        
        <li><a href="<?php echo base_url() ?>page/header/catering_detail">Catering Detail</a></li>        
        <li><a href="<?php echo base_url() ?>page/header/in_the_kitchen">In The Kitchen</a></li>        
        <li><a href="<?php echo base_url() ?>page/header/contact">Contact</a></li>        
      </ul>
    </li>
    <hr> 

    <li>
      <a data-toggle="collapse" data-parent="#accordion" href="#collapseNinetyNine">Set Fonts</a>
      <ul id="collapseNinetyNine" class="panel-collapse collapse ">        
        <li><a href="<?php echo base_url() ?>fonts/navigation">Navigation Bar</a></li>        
        <li><a href="<?php echo base_url() ?>fonts/home_slider">Home Slider</a></li>        
        <li><a href="<?php echo base_url() ?>fonts/section">Section Fonts</a></li>        
        <li><a href="<?php echo base_url() ?>fonts/space">Space Fonts</a></li>        
        <li><a href="<?php echo base_url() ?>fonts/space_detail">Space Detail Fonts</a></li>        
        <li><a href="<?php echo base_url() ?>fonts/catering">Catering Fonts</a></li>        
        <li><a href="<?php echo base_url() ?>fonts/catering_detail">Catering Detail Fonts</a></li>        
        <li><a href="<?php echo base_url() ?>fonts/news">News Fonts</a></li>        
        <li><a href="<?php echo base_url() ?>fonts/news_detail">News Detail Fonts</a></li>        
        <li><a href="<?php echo base_url() ?>fonts/contact">Contact Fonts</a></li>        
        <li><a href="<?php echo base_url() ?>fonts/footer_inquire">Footer Inquire Form</a></li>        
        <li><a href="<?php echo base_url() ?>fonts/footer_connect_container">Footer Connect</a></li>        
        <li><a href="<?php echo base_url() ?>fonts/footer">Footer</a></li>        
      </ul>
    </li>
    <hr> 

    <li><a href="<?php echo base_url() ?>about/all">About</a></li>
    <!--hr>
    <li><a href="<?php echo base_url() ?>appointments/all">Appointments</a></li-->
    <hr>
    <li><a href="<?php echo base_url() ?>news/all">News</a></li>  
    <hr>  
    <li style="display:none"><a href="<?php echo base_url() ?>team/all">Team</a></li>  
    <hr>  
    <li><a href="<?php echo base_url() ?>catering/all">Catering</a></li>  
    <hr>  
    <!-- <li><a href="<?php echo base_url() ?>catering_partners/all">Catering Partners</a></li>  
    <hr>   -->
    <li><a href="<?php echo base_url() ?>sections/all">Sections</a></li>  
    <hr>  
    <li><a href="<?php echo base_url() ?>categories/all">Categories</a></li>
    <hr>   
    <li><a href="<?php echo base_url() ?>contact/all">Contact Form</a></li>  
    <hr>
    <li><a href="<?php echo base_url() ?>event_inquiry/all">Event Inquiry</a></li>  
    <hr>
    <li><a href="<?php echo base_url() ?>newsletter/all">News Letter</a></li>  
    <hr>
    <li><a href="<?php echo base_url() ?>gallery/all">Gallery</a></li>  
    <hr>
    <li><a href="<?php echo base_url() ?>team/all">Team</a></li>  
    <hr>
    <!--li><a href="<?php echo base_url() ?>admin/change_fonts">Change Fonts</a></li>
    <hr-->
    <li><a href="<?php echo base_url() ?>admin/site_content">Site Content</a></li>  
    <hr>  
    
    <li><a href="<?php echo base_url() ?>page/all">Pages</a></li>
 
<!--     <hr>
    <li><a href="<?php echo base_url() ?>admin/social_links">Social Links</a></li>
 --> 
    <!--hr> 
    <li><a href="<?php echo base_url() ?>home/headerimages">Headers</a></li-->
    
    <hr> 

    <li>
      <a data-toggle="collapse" data-parent="#accordion" href="#collapseThree">Seo Pages</a>
      <ul id="collapseThree" class="panel-collapse collapse ">        
        <li><a href="<?php echo base_url() ?>seo/contents/index">Home</a></li>
        <hr>
        <li><a href="<?php echo base_url() ?>seo/contents/space">Space</a></li>
        <hr>
        <li><a href="<?php echo base_url() ?>seo/contents/about">About</a></li>
        <hr>
        <li><a href="<?php echo base_url() ?>seo/contents/catering">Catering</a></li>
        <hr>
        <li><a href="<?php echo base_url() ?>seo/contents/news">News</a></li>
        <hr>
        <li><a href="<?php echo base_url() ?>seo/contents/contact">contact</a></li>
        <hr>
      </ul>
    </li>
    <hr>

    <!--<li><a href="<?php echo base_url() ?>page_content/all">Page Content</a></li>
    <hr>-->    
  </ul>

</div>

<style type="text/css">

  .sidebar{

    background-color: #FFF;

    overflow-y: scroll;
    
    overflow-x: hidden;
  }

  hr{

    margin: 0px;

  }

  .panel-collapse{
    list-style: none;
    padding-left:15px;
  }

  ul.panel-collapse li a{
    padding: 0px 15px;
    line-height: 25px;
  }

</style>